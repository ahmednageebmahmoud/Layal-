...
{ Id: null, PackageName: Token.select }];
    ...
    //-=-=-=-=-=-=-= Saves ==-=-=-=-=-=-=-=
    s.taskFinsh = event=> {
        bootstrapModelShow('taskFinsed');
        s.taskFinshDetail={EventId : event.Id};
        return;
    }
    ...
    s.saveTaskFinsh = frm=> {
        if(frm.$invalid)
        {
            s.taskFinshDetailSubmitErro = true;
            return;
        }

        s.taskFinshDetailSubmitErro = false;

        //show confirm delete
        SMSSweet.delete(() => {
            //is approve must be deleted
            BlockingService.block();
            eventsServ.taskFinsh(s.taskFinshDetail).then(d => {
